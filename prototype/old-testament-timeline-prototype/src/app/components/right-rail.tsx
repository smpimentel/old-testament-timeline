...
export function RightRail({ entity, onClose, onViewRelationship, pathMode }: RightRailProps) {
  ...
  // Format date
  const formatDate = (year: number, certainty: 'exact' | 'approximate') => {
        ...
        >
          <div className="flex-1 min-w-0">
            ...
            >
              ...
              {entity.type === 'event' && entity.category}
              {entity.type === 'book' && entity.genre}
            </div>
            <h2 
          ...
          <button
            onClick={onClose}
            className="flex-shrink-0 p-2 rounded-md transition-all hover:scale-110"
            style={{
              ...
              <div className="space-y-2">
                {relatedEntities.map(related => (
                  <button
                    ...
                    disabled={!pathMode}
                    className="w-full px-3 py-2 rounded-md flex items-center justify-between transition-all text-left disabled:opacity-40 disabled:cursor-not-allowed"
                    style={{
                      border: '1px solid var(--color-base-grid-major)',
                      background: 'var(--color-base-surface-panel)',
                    }}
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      >
                        {related.type === 'person' && related.role}
                        {related.type === 'event' && related.category}
                        {related.type === 'book' && related.genre}
                      </div>
                    </div>
                    {pathMode && (
                      <ArrowRight 
